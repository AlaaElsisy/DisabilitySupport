...
namespace DisabilitySupport.DAL.Interfaces
{
    public interface IHelperServiceRepository:IGenericRepository<HelperService>
    {
        ...
        Task<(List<HelperService> Items, int TotalCount)> GetPagedAsync( int? helperId,  int? serviceCategoryId,  string? searchWord, decimal? minBudget, decimal? maxBudget, string? sortBy, int pageNumber,  int pageSize );
    }
}
